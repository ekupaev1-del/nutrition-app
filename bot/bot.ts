...
ВАЖНО: Если текст не про еду, верни isFood: false с описанием и дружелюбным сообщением. Если это еда — оцени количество и определи калорийность и макроэлементы.`;
    ...
    if (parsed.isFood === false) {
      ...
      const notFoodResponse: NotFoodResponse = {
        isNotFood: true,
...
**3. Что изменить** (краткий список)`;
    ...
    if (selectError) {
      console.error("[bot] Ошибка проверки пользователя при обработке текста:", selectError);
      await ctx.telegram.editMessageText(
        ctx.chat!.id,
        processingMsg.message_id,
        undefined,
        "❌ Ошибка базы данных. Попробуйте позже или используйте /start."
      );
      return;
    }
    ...
    if (!existingUser) {
      ...
      console.log(`[bot] Пользователь не найден, создаем для telegram_id: ${telegram_id}`);
      ...
      if (createError) {
        ...
        await ctx.telegram.editMessageText(
          ...
          processingMsg.message_id,
          undefined,
          `❌ Ошибка создания пользователя: ${createError.message}. Попробуйте /start для регистрации.`
        );
        return;
      }
      ...
      if (!newUser || !newUser.id) {
        ...
        await ctx.telegram.editMessageText(
          ...
          processingMsg.message_id,
          undefined,
          "❌ Ошибка: не удалось создать пользователя. Попробуйте /start для регистрации."
        );
        return;
      }

      console.log(`[bot] Пользователь успешно создан, id: ${newUser.id}`);
    }
...
bot.on("photo", async (ctx) => {
  try {
    ...
    if (selectError) {
      console.error("[bot] Ошибка проверки пользователя при обработке фото:", selectError);
      await ctx.telegram.editMessageText(
        ctx.chat!.id,
        processingMsg.message_id,
        undefined,
    ...
    // Формируем ответ (mealAnalysis уже определен выше как type guard)
    const response = `✅ Добавлено:\n${mealAnalysis.description}\n🔥 ${mealAnalysis.calories} ккал | 🥚 ${mealAnalysis.protein.toFixed(1)}г | 🥥 ${mealAnalysis.fat.toFixed(1)}г | 🍚 ${mealAnalysis.carbs.toFixed(1)}г\n\n${formatProgressMessage(todayMeals, dailyNorm)}`;
...
bot.on("voice", async (ctx) => {
  try {
    ...
    if (selectError) {
      console.error("[bot] Ошибка проверки пользователя при обработке голоса:", selectError);
      await ctx.telegram.editMessageText(
        ctx.chat!.id,
        processingMsg.message_id,
        undefined,
