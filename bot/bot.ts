...
bot.on("text", async (ctx) => {
  try {
    ...
    // Проверяем, что это не первое нажатие на "✏️ Редактировать прием пищи"
    // (первое нажатие уже обработано выше и показывает подменю)
    // Это второе нажатие, которое открывает редактор
    if (text === "✏️ Редактировать прием пищи") {
      // Проверяем, есть ли уже подменю (если пользователь нажал второй раз)
      // Получаем userId для Mini App
      const { data: user } = await supabase
        .from("users")
        .select("id")
