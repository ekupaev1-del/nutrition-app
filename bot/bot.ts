...
bot.on("text", async (ctx) => {
  try {
    ...
    if (text === "✏️ Обновить анкету") {
      ...
      const statsUrl = `https://nutrition-app4.vercel.app/stats?id=${user.id}`;
      // Обновляем меню с прямой ссылкой на Mini App
      return ctx.reply(" ", {
        reply_markup: {
          keyboard: [
            [
              { text: "✏️ Обновить анкету", web_app: { url } }
            ],
            [
              { text: "📋 Получить отчет", web_app: { url: statsUrl } }
            ],
    ...
    // Кнопка "📋 Получить отчет" теперь напрямую открывает Mini App через web_app в keyboard button
    // Обработчик текста не нужен, так как кнопка не отправляет текст при нажатии

    if (text === "✏️ Редактировать прием пищи") {
      // Проверяем, есть ли уже подменю (по наличию последнего сообщения с подменю)
      // Если это первое нажатие - показываем подменю
      // Если это второе нажатие - открываем редактор
      ...
      // Показываем подменю с двумя опциями
      return ctx.reply("Выберите действие:", {
        reply_markup: {
          keyboard: [
            ...
            [
              { text: "📝 Открыть редактор приемов пищи" }
            ],
            [
              { text: "🔙 Назад в меню" }
            ]
    ...
    if (text === "📝 Открыть редактор приемов пищи") {
      // Получаем userId для Mini App
      const { data: user } = await supabase
        .from("users")
        .select("id")
