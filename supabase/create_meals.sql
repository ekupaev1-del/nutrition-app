-- Создание таблицы meals для хранения приёмов пищи
CREATE TABLE IF NOT EXISTS public.meals (
  ...
  -- Связь с users через telegram_id (не через id, так как users.id может быть другим типом)
  CONSTRAINT fk_meals_telegram_id FOREIGN KEY (telegram_id) 
...
COMMENT ON COLUMN public.meals.telegram_id IS 'Telegram ID пользователя (связь с users.telegram_id)';
